...
class RcxContent {
    ...
    disableTab() {
        if (window.rikaichan !== undefined) {
            ...
            if (e === null || e === void 0 ? void 0 : e.parentNode) {
    ...
    showPopup(text, elem, x = 0, y = 0, looseWidth) {
        ...
        if (!popup) {
            const css = topdoc.createElementNS('http://www.w3.org/1999/xhtml', 'link');
            css.setAttribute('rel', 'stylesheet');
            css.setAttribute('type', 'text/css');
            const color = window.rikaichan.config.popupcolor;
            css.setAttribute('href', chrome.extension.getURL('css/popup-' + color + '.css'));
        ...
        if (this.getContentType(topdoc) === 'text/plain') {
            ...
            docFragment.appendChild(document.createElementNS('http://www.w3.org/1999/xhtml', 'span'));
            docFragment.firstChild.innerHTML = text;
    ...
    isInline(node) {
        return (Object.prototype.hasOwnProperty.call(this.inlineNames, node.nodeName) ||
    ...
    show(tdata, dictOption) {
        ...
        this.lastRo = ro;
        chrome.runtime.sendMessage({ type: 'xsearch', text: text, dictOption: String(dictOption) }, this.processEntry);
        return 1;
    }
    ...
    getFirstTextChild(node) {
        return document
            .evaluate('descendant::text()[not(parent::rp) and not(ancestor::rt)]', node, null, XPathResult.ANY_TYPE, null)
            .iterateNext();
        //
    }
    makeFake(real) {
    ...
    tryUpdatePopup(ev) {
        ...
        if ((window.rikaichan.config.showOnKey.includes('Alt') &&
            !ev.altKey &&
            !altGraph) ||
        ...
        try {
            if (eventTarget.nodeName === 'TEXTAREA' ||
        ...
        if (tdata.title) {
            tdata.popX = ev.clientX;
            tdata.popY = ev.clientY;
            tdata.timer = window.setTimeout((tdata, title) => {
